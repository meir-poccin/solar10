...
const address = localStorage.getItem('addres')
const bill = localStorage.getItem('bill')
const kwh = localStorage.getItem('cost')
const commercial = localStorage.getItem('stat')

//setting the default radio button to commercial
const getDefault = (commercia) => {  
    let defaults 
    if(commercia == 'commercial'){
      defaults = .12
    }else{
      defaults = .25
    }
...
const showHide = () =>{ 
  ...
  document.querySelector('body').style.backgroundColor = '#997000'
...
const savings = (month, year) => {
    ...
    let cost_kwh
    kwh > 0 ? cost_kwh = year : 
    cost_kwh = getDefault(localStorage.getItem('stat'))
    console.log('first')
    ...
    document.querySelector('.addr1').innerText = address
    document.querySelector('.addr2').innerText = address
    document.querySelector('.date').innerText = dates  
    chart1(address, system_size)
   console.log(address)
   chart2(Number(bill), Number(kwh))
...
/*
        function maps(add){
            map.innerHTML = '<img src="https://maps.googleapis.com/maps/api/staticmap?center='+add+'&zoom=17&maptype=satellite&size=300x150&scale=2&&key=AIzaSyDOSvt2i5gBh20Mu1VpDsPgjes_3OxTmBk" alt="map">'
        }
        */
   function maps(add){
   map.innerHTML = 
   '<iframe width="600" height="300" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.com/maps?width=100%25&amp;height=600&amp;hl=en&amp;q='+add+'&amp;t=&amp;z=14&amp;ie=UTF8&amp;iwloc=B&amp;output=embed"><a href="https://www.maps.ie/distance-area-calculator.html">measure area map</a></iframe>'
   }
...
const getAcAnnual = (annual_production) =>{
   ...
   const chart1 = (address,systemCapacity)=>{    
    fetch('https://developer.nrel.gov/api/pvwatts/v8.json?api_key=3tqiqGVtlbyrfadqyJK0TuxtDrWKF7eA48yUppK4&azimuth=180&system_capacity='+systemCapacity+'&losses=14&array_type=1&module_type=0&gcr=0.4&dc_ac_ratio=1.2&inv_eff=96.0&radius=0&dataset=nsrdb&tilt=10&address='+address+'&soiling=12|4|45|23|9|99|67|12.54|54|9|0|7.6&albedo=0.3&bifaciality=0.7'
        ...
        .then((data) =>  getAcAnnual(data.outputs.ac_annual *1.2))
...
const getSavings = () =>{
  /*if(!bill.value || isNaN(bill.value) || (isNaN(kwh.value) && kwh.value)||
      ...
      }else  if(validAddress(localStorage.getItem('addres'))!== 'USA'){
         alert('Please select USA address from autocomplete')
         reset()
      }else{ */
      format()
 savings(Number(bill), Number(kwh))
